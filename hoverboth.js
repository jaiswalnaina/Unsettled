...
let currentlyHovering = false; // This will be set to true if hovering over any element
let numCategories = 8;
let numYears = 10;
...
let textLines = [
  'In the past decade, an average of 20 million individuals annually',
  'have been displaced from their homes, a stirring consequence',
  'of the escalating natural disasters fueled by climate change',
  'and global warming.'
];
...
function drawParticles() {
  particles.forEach(p => {
      ...
      if (d < p.radius) {
          hoverParticle = p;
          hoveredCategory = p.category; // Set hover state
      }

      let alpha = (!hoveredCategory || hoveredCategory === p.category) ? 255 : 50;
      let particleColor = color(hazardColors[hazard.indexOf(p.category)]);
      particleColor.setAlpha(alpha);
      fill(particleColor);
      ellipse(p.x, p.y, p.radius, p.radius);
  });

  if (hoverParticle && dist(mouseX, mouseY, hoverParticle.x, hoverParticle.y) >= hoverParticle.radius) {
      hoverParticle = null; 
      hoveredCategory = null; // Clear hover state if the mouse moves away from the particle
  }

  
}

// Calculate the maximum total across all years for scaling purposes
function calculateMaxYearTotal() {
  let maxTotal = 0;
  for (let year of years) {
      if (yearTotals[year] > maxTotal) {
          maxTotal = yearTotals[year];
      }
  }
  return maxTotal;
}
...
function drawStackedBarChart() {
  let chartX = 150;
  let barWidth = (width - 3700) / numYears;
  let spacing = 25;
  let chartTopMargin = 1500;
  let chartBottomMargin = 700;
  let maxBarHeight = height - chartTopMargin - chartBottomMargin;
  ...
  for (let yearIndex = 0; yearIndex < numYears; yearIndex++) {
      ...
      let y = height - chartBottomMargin;

      for (let categoryIndex = 0; categoryIndex < numCategories; categoryIndex++) {
          let value = dataValues[categoryIndex][yearIndex];
          let barHeight = (value / yearTotals[year]) * maxBarHeight;
          let fillColor = color(hazardColors[categoryIndex]);

          if (mouseX > x && mouseX < x + barWidth && mouseY > y - barHeight && mouseY < y) {
              hoveredCategory = hazard[categoryIndex]; // Detect hover over bar
          }

          fillColor.setAlpha((!hoveredCategory || hoveredCategory === hazard[categoryIndex]) ? 255 : 50);
          fill(fillColor);
          rect(x, y - barHeight, barWidth, barHeight);
          y -= barHeight;
      }
      if (hoveredCategory && dist(mouseX, mouseY, hoveredCategory.x, hoveredCategory.y) >= hoveredCategory) {
        hoverParticle = null; 
        hoveredCategory = null; // Clear hover state if the mouse moves away from the particle
    }
    // Draw the year label
    ...
    text(year, x + barWidth / 2, height - 600);
  }
}
...
function drawLegend() {
    ...
    let padding = 150;  // Additional spacing between items to prevent overlap

    textSize(40);  // Set the text size for the legend
