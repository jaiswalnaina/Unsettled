...
function draw() {
  background(235, 243, 255); // Redraw background to clear old frames
...
function drawStackedBarChart() {
  ...
  let anyHovered = false; // To track if any bar is hovered
  let hoveredCategory = null;
  ...
  for (let yearIndex = 0; yearIndex < numYears; yearIndex++) {
      ...
      for (let categoryIndex = 0; categoryIndex < numCategories; categoryIndex++) {
          ...
          fillColor.setAlpha((anyHovered && hoveredCategory === hazard[categoryIndex])? 255 : 50);
          fill(fillColor);
          rect(x, y - barHeight, barWidth, barHeight);
          y -= barHeight;
      } 
      
      if (hoveredCategory && dist(mouseX, mouseY, hoveredCategory.x, hoveredCategory.y) >= hoveredCategory.size) {
        hoverParticle = null;
        hoveredCategory = null; // Clear hover state if the mouse moves away from the particle
      }
  }
